...
The pipeline runs in up to 4 steps, each independently toggleable via `src/config.yaml`:

```
Step 1: Dataset Processing     — preprocess raw logs, extract features, split data
Step 2: Testing & Evaluation   — train or test an IDS model
Step 3: Adversarial Perturbation — generate adversarial examples using attack methods
Step 4: Robust Training        — retrain model on clean + adversarial samples
```
...
```yaml
run_steps:
  dataset_processing: false       # Step 1
  testing_and_evaluation: false   # Step 2
  adversarial_perturbation: true  # Step 3
  robust_training: true           # Step 4
...
The pipeline will execute only the steps with their `run_steps` flag set to `true`.
